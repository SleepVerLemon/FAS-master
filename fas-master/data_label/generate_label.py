...
data_dir = '/root/SSDG/data/mtcnn_prodatasets/'

def msu_process():
    test_list = []
    # data_label for msu
    for line in open('/root/SSDG/data/original_data/MSU-MFSD/test_sub_list.txt', 'r'):
        test_list.append(line[0:2])
    train_list = []
    for line in open('/root/SSDG/data/original_data/MSU-MFSD/train_sub_list.txt', 'r'):
    ...
    path_list = glob.glob(dataset_path + '**/*.png', recursive=True)  #该方法返回所有匹配的文件路径列表（list）
    path_list.sort() #进行排序
    for i in range(len(path_list)):
        ...
        dict['photo_label'] = label     #建立dict放入图片path和label(真为1假为0)
        #video_num = path_list[i].split('/')[-2].split('_')[0]   #01_real？
        video_num = path_list[i].split('/')[-1].split('_')[1]   #01_real？
        video_num = video_num[-2:]
...
def casia_process():
    ...
    path_list.sort()
    for i in range(len(path_list)):
        flag = path_list[i].split('/')[-1].split('.')[0]
        str = '_'
        flag = flag[flag.index(str):]
        flag = flag[1:]
        print("flagxxxxxxxxx", flag)
...
def oulu_process():
    ...
    path_list.sort()
    for i in range(len(path_list)):
        flag = int(path_list[i].split('/')[-1].split('_')[-1][0])
        if (flag == 1):
            label = 1
        else:
            label = 0
