...
def train():
    ...
    # cosine_schedule = optim.lr_scheduler.ExponentialLR(optimizer=optimizer, gamma=0.95)  #C
    ...
    for iter_num in range(max_iter+1):
        ...
        # if (epoch <= config.lr_epoch_1):# 
        #     cosine_schedule = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer,T_max=6000,eta_min=1e-9) 
        # else:
        # adjust_learning_rate2(optimizer, epoch, init_param_lr, config.lr_epoch_1, config.lr_epoch_2, config.lr_epoch_3)
        ######### data prepare #########
        ...
        # input_data = torch.cat([src1_img_real, src1_img_fake], dim=0)
        source_real_label = torch.cat([src1_label_real, src2_label_real, src3_label_real], dim=0)
        # source_label = torch.cat([src1_label_real, src1_label_fake], dim=0)
